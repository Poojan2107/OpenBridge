...
const PREFLIGHT_QUESTIONS: Question[] = [
  {
    ...
    answerIndex: 1,
    explanation:
      "DCO is a lightweight process used by GSoC, LF, and large Git repos (like Linux, Docker, or CNCF) where you certify you wrote the code by certifying your email in the commit header with git commit - s. It avoids tedious legal contracts.",
  },
  {
    id: 2,
    question:
      "When should you prefer Git's 'Squash and Merge' option over a standard Fast-Forward Merge?",
    options: [
    ...
    answerIndex: 1,
    explanation:
      "Squashing combines all commits from a feature branch into one solid commit in the master line, making the git tree highly legible, clear, and professional.",
  },
  {
    id: 3,
    question:
      "What is the primary difference between a Permissive license (MIT/Apache) and a Copyleft license (GPL)?",
    options: [
    ...
    answerIndex: 2,
    explanation:
      "GPL (Copyleft) specifies that any software modifying or matching GPL blocks must release their modifications under the same open conditions. MIT or Apache permit private redistribution.",
  },
  {
    id: 4,
    question:
      "If your pull request is marked 'stale' by a GitHub Actions bot, what is the correct practice?",
    options: [
    ...
    answerIndex: 1,
    explanation:
      "Rebasing refreshes the timeline, solves conflict errors, and tells the maintainers you are active. Avoid toxic or demanding reactions.",
  },
];
...
export default function PreflightConsole() {
  ...
  const [generatedGPG, setGeneratedGPG] = useState<string | null>(
    () => localStorage.getItem("ob_gpg_key") || null,
  );
  ...
  const [submittedQuiz, setSubmittedQuiz] = useState(
    () => localStorage.getItem("ob_quiz_submitted") === "true",
  );
  ...
  const [systemChecks, setSystemChecks] = useState<
    Array<{ name: string; status: "success" | "warning"; details: string }>
  >(() => {
    try {
      ...
      if (saved) {
        ...
        if (!parsed.some((c: any) => c.name === "Git Workflow DCO Compliance")) {
          parsed.push({
            name: "Git Workflow DCO Compliance",
            status: "warning",
            details:
              "Unverified Git workflow sequence. Complete git commits in Sandbox Terminal to test.",
          });
        }
        return parsed;
      }
    ...
    return [
      {
        ...
        status: "success",
        details: "Local signature mapped to validated email records.",
      },
      {
        name: "Repository Origin Trust",
        status: "success",
        details: "Push boundaries aligned to upstream main forks.",
      },
      {
        name: "Commit Auth (GPG Keys)",
        status: "warning",
        details: "Unsigned timeline commits detected. Generate GPG below to secure.",
      },
      {
        name: "Git Workflow DCO Compliance",
        status: "warning",
      ...
      prev.map((c) =>
        c.name === "Git Workflow DCO Compliance"
          ? {
              name: "Git Workflow DCO Compliance",
              status: "success",
              details: "Git staging, sign-offs, and push operations verified successfully!",
            }
          : c,
      ),
    );
        ...
        prev.map((c) =>
          c.name === "Commit Auth (GPG Keys)"
            ? {
                name: "Commit Auth (GPG Keys)",
                status: "success",
                details: `GPG Key 0x${longId.substring(0, 8)} successfully linked & signed.`,
              }
            : c,
        ),
      );
          ...
          prev.map((c) =>
            c.name === "Commit Auth (GPG Keys)"
              ? {
                  name: "Commit Auth (GPG Keys)",
                  status: "success",
                  details: `GPG Key 0x${data.metadata.keyId.substring(0, 8)} (${data.metadata.name}) verified & linked successfully.`,
                }
              : c,
          ),
        );
      ...
      <div className="border border-zinc-90 w-full bg-zinc-950/40 rounded-xl p-6 relative overflow-hidden">
        <div className="mb-4">
          ...
          <p className="text-zinc-400 text-xs mt-1.5 max-w-3xl leading-relaxed">
            Open-source communities like Google Summer of Code (GSoC) and Hacktoberfest mandate
            clean contribution standards. Validate your Git identity, verify your commit signature,
            and test your open-source hygiene prior to code submission.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-[#090a0f] border border-zinc-900 rounded-xl p-5 hover:border-zinc-800 transition-all duration-150 relative">
            ...
            <p className="text-zinc-500 text-[11px] mt-1.5 leading-relaxed">
              Before submitting Pull Requests, modern systems require GPG Commit Signing or explicit
              sign-offs in your local Git environments. Check your validation matrix:
            </p>
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-zinc-300 flex items-center gap-2">
                      <span
                        className={`inline-block w-1.5 h-1.5 rounded-full ${check.status === "success" ? "bg-emerald-500" : "bg-amber-500"}`}
                      ></span>
                      {check.name}
                    </span>
                    <span
            ...
            <div className="mt-6 border-t border-zinc-90 pt-5 pr-1 space-y-4">
              ...
              {gpgTab === "generate" ? (
                ...
                <form onSubmit={handleGenerateGPG} className="space-y-3">
                  <span className="block text-[10px] uppercase font-bold tracking-wider text-zinc-500 font-mono">
                    Sign Your Identity (Secure GPG Key Generator)
                  </span>

                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[9px] text-zinc-550 mb-1 font-mono">
                        Full Git Legal Name
                      </label>
                      <input
                        type="text"
                ...
                <form onSubmit={handleVerifyGPG} className="space-y-3">
                  <span className="block text-[10px] uppercase font-bold tracking-wider text-zinc-500 font-mono">
                    Verify GPG Public Key Block
                  </span>
                  <div>
                    <label className="block text-[9px] text-zinc-550 mb-1 font-mono">
                      Paste GPG Public Key ASCII Armor
                    </label>
                    <textarea
                      required
                      value={pastedGpgKey}
                  ...
                  {parsedMetadata && (
                    <div className="p-3 rounded bg-blue-950/10 border border-blue-900/30 text-zinc-300 font-mono text-[10.5px] space-y-1">
                      <div className="text-[9px] uppercase font-bold text-sky-400 mb-1">
                        Parsed OpenPGP Metadata
                      </div>
                      <div>
                        <span className="text-zinc-500">Key ID:</span>{" "}
                        <span className="text-zinc-200 font-bold">0x{parsedMetadata.keyId}</span>
                      </div>
                      <div>
                        <span className="text-zinc-500">Identity:</span>{" "}
                        <span className="text-zinc-200">
                          {parsedMetadata.name} &lt;{parsedMetadata.email}&gt;
                        </span>
                      </div>
                      <div>
                        <span className="text-zinc-500">Algorithm:</span>{" "}
                        <span className="text-zinc-200">
                          {parsedMetadata.algorithm} ({parsedMetadata.keyLength} bits)
                        </span>
                      </div>
                      <div>
                        <span className="text-zinc-500">Created At:</span>{" "}
                        <span className="text-zinc-200">
                          {new Date(parsedMetadata.createdAt).toLocaleDateString()}
                        </span>
                      </div>
                    </div>
                  )}
              ...
              {generatedGPG && gpgTab === "generate" && (
                <div className="mt-4 border border-zinc-90 w-full rounded overflow-hidden">
                  ...
                  <div className="p-2.5 bg-zinc-950 text-[10px] text-zinc-500 font-mono flex items-center gap-2.5 border-t border-zinc-90">
                    ...
                    <span>
                      Use git commit -S and paste this signed DCO block parameters to show the
                      verified green commit badge in GitHub!
                    </span>
                  </div>
                </div>
              )}
          ...
          <div className="bg-[#090a0f] border border-zinc-900 rounded-xl p-5 hover:border-zinc-800 transition-all duration-150 relative">
            ...
            <p className="text-zinc-500 text-[11px] leading-relaxed mb-4 font-mono">
              Practice making signed DCO commits on a mock terminal before pushing to upstream. Type{" "}
              <code className="text-zinc-300">help</code> or{" "}
              <code className="text-zinc-300">git status</code> to begin.
            </p>
        ...
        <div className="lg:col-span-7 bg-[#090a0f] border border-zinc-900 rounded-xl p-5 hover:border-zinc-800 transition-all duration-150 flex flex-col justify-between">
          <div>
            ...
            <p className="text-zinc-500 text-[11px] mt-1.5 leading-relaxed">
              Test your proficiency in git version control, merge behaviors, open source licensing,
              and pull request communication. Earn a score above 75% to unlock your simulated
              Onboarding Badge.
            </p>
            ...
            <div className="space-y-5 mt-5 max-h-[460px] overflow-y-auto pr-1">
              {PREFLIGHT_QUESTIONS.map((q, qIdx) => {
                ...
                return (
                  <div
                    key={q.id}
                    className="p-4 rounded-lg bg-zinc-950 border border-zinc-90 w-full space-y-3"
                  >
                      ...
                      {q.options.map((opt, optIdx) => {
                        ...
                        if (submittedQuiz) {
                          if (optIdx === q.answerIndex) {
                            itemClass =
                              "bg-emerald-950/20 text-emerald-300 border-emerald-900/80 font-semibold";
                        ...
                        return (
                          ...
                          >
                            <span className="font-semibold text-[10px] text-zinc-500 font-mono mt-0.5">
                    ...
                    {submittedQuiz && (
                      <div className="mt-2.5 pl-5 pt-3 border-t border-zinc-90 flex items-start gap-2 text-[10.5px] leading-relaxed select-none">
                        ...
                        <div>
                          <span
                            className={`font-bold font-mono ${isCorrect ? "text-emerald-400" : "text-amber-400"} uppercase block text-[9px] tracking-wider mb-0.5`}
                          >
            ...
            {!submittedQuiz ? (
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 bg-zinc-950 rounded-lg border border-zinc-900 select-none">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2.5 rounded ${score >= 3 ? "bg-emerald-950/20 text-emerald-450 border border-emerald-900/40" : "bg-zinc-90 text-zinc-400 border border-zinc-800"}`}
                  >
