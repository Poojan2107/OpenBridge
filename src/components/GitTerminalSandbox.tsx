...
const CHALLENGES: ChallengeInfo[] = [
  {
    ...
    badge: "DCO Sign-off",
    description:
      "Certify that you wrote the code you are submitting by signing your commit. Modern repos block unsigned commits.",
    steps: [
  ...
  },
  {
    ...
    description:
      "When pushing changes, you might get rejected because someone else pushed first. Learn to pull and resolve conflicts.",
    steps: [
  ...
  },
  {
    ...
    description:
      "Keep the git history clean. Squash multiple messy work-in-progress (WIP) commits into a single clean commit before merging.",
    steps: [
...
export default function GitTerminalSandbox({ onSuccess }: GitTerminalSandboxProps) {
  ...
  const resetChallengeState = (challengeId: number, userInitiated = true) => {
    ...
    if (challengeId === 1) {
      lines.push(
        ...
        {
          text: "Modified files ready in working tree: src/App.tsx, verify-pr.yml",
          type: "stdout",
        },
        {
          text: "Goal: Stage, commit with DCO sign-off (-s flag), and push your work.",
          type: "stdout",
        },
      );
      ...
      lines.push(
        ...
        {
          text: "Modified file in working tree: src/App.tsx (you have local edits to push, but origin is ahead).",
          type: "stdout",
        },
        {
          text: "Goal: Stage, commit, push, pull remote changes, resolve conflicts, and push successfully.",
          type: "stdout",
        },
      );
    } else {
      lines.push(
        ...
        {
          text: "You have 3 separate commits locally: 1 feature commit and 2 messy WIP edits.",
          type: "stdout",
        },
        {
          text: "Goal: Squash local history into 1 clean commit using interactive rebase, then force push.",
          type: "stdout",
        },
      );
    }
  ...
  const handleCommand = (rawCmd: string) => {
    ...
    if (primary === "help") {
      newLines.push(
        ...
        {
          text: "  git status                      - View modified and staged files",
          type: "stdout",
        },
        { text: "  git add .                       - Stage all modified files", type: "stdout" },
        { text: '  git commit -m "<msg>"           - Commit changes (local only)', type: "stdout" },
        {
          text: '  git commit -s -m "<msg>"        - Commit changes with DCO sign-off signature',
          type: "stdout",
        },
        {
          text: "  git pull                        - Pull changes from remote origin repository",
          type: "stdout",
        },
        {
          text: "  git push                        - Push commits to remote origin",
          type: "stdout",
        },
        {
          text: "  git log                         - View local commit history log",
          type: "stdout",
        },
        {
          text: "  cat <file>                      - View contents of a file (e.g., cat src/App.tsx)",
          type: "stdout",
        },
        {
          text: "  git checkout --ours <file>      - Resolve conflict choosing local edits",
          type: "stdout",
        },
        {
          text: "  git checkout --theirs <file>    - Resolve conflict choosing remote edits",
          type: "stdout",
        },
        {
          text: "  git rebase -i HEAD~3            - Launch interactive rebase to squash commits",
          type: "stdout",
        },
        {
          text: "  git push --force (or -f)        - Force push to overwrite remote history",
          type: "stdout",
        },
        {
          text: "  clear                           - Clear the terminal screen buffer",
          type: "stdout",
        },
      );
    ...
    if (primary !== "git" && primary !== "cat") {
      newLines.push({
        text: `sh: command not found: ${primary}. Type 'help' for supported commands.`,
        type: "stderr",
      });
      setHistory((prev) => [...prev, ...newLines]);
      setInputVal("");
      ...
      if (!targetFile) {
        ...
        if (activeChallenge === 2 && gitState.c2ConflictActive && !gitState.c2ConflictResolved) {
          newLines.push(
            ...
            {
              text: "const WelcomeMessage = () => <h1>Welcome to OpenBridge Hub!</h1>;",
              type: "input",
            },
            { text: "=======", type: "stderr" },
            {
              text: "const WelcomeMessage = () => <h1>Welcome, Contributor!</h1>;",
              type: "stdout",
            },
    ...
    if (!sub) {
      newLines.push({
        text: "Error: 'git' requires a subcommand. Type 'help' for available actions.",
        type: "stderr",
      });
      setHistory((prev) => [...prev, ...newLines]);
      setInputVal("");
    ...
    if (activeChallenge === 1) {
      if (sub === "status") {
        ...
        if (!gitState.staged) {
          newLines.push({
            text: "Error: Nothing staged to commit. Run 'git add .' first.",
            type: "stderr",
          });
        } else if (gitState.committed) {
          ...
          if (hasSignoff) {
            ...
            newLines.push(
              ...
              {
                text: " Signed-off-by: Guest Committer <guest-committer@example.com>",
                type: "success",
              },
            );
          } else {
            newLines.push(
              ...
              {
                text: "Reason: You must certify your Developer Certificate of Origin (DCO) by adding a sign-off footer to your commit message.",
                type: "stderr",
              },
              {
                text: "Solution: Run 'git commit -s -m \"<message>\"' to auto-sign.",
                type: "stderr",
              },
            );
          }
        ...
        if (!gitState.committed) {
          newLines.push({
            text: "Error: No local commits to push. Run 'git commit' first.",
            type: "stderr",
          });
        } else if (!gitState.signed) {
          newLines.push({
            text: "Error: Unsigned commits detected. Rebase or commit with '-s' sign-off.",
            type: "stderr",
          });
        } else {
          ...
          newLines.push(
            ...
            {
              text: "To https://github.com/guest-committer/openbridge-onboarding-hub.git",
              type: "stdout",
            },
        ...
        newLines.push({
          text: `git: subcommand '${sub}' is not supported in Challenge 1.`,
          type: "stderr",
        });
      }
    }
    ...
    // ==========================================
      ...
      if (sub === "status") {
        ...
        if (arg === "." || arg === "src/App.tsx") {
          ...
          newLines.push({
            text: "Error: Specify file to stage (e.g. 'git add src/App.tsx')",
            type: "stderr",
          });
        }
      } else if (sub === "commit") {
        if (gitState.c2ConflictActive && !gitState.c2ConflictResolved) {
          newLines.push({
            text: "Error: Cannot commit while conflicts are active. Resolve them first.",
            type: "stderr",
          });
        } else if (gitState.c2ConflictResolved && !gitState.c2StagedResolved) {
          newLines.push({
            text: "Error: Stage your conflict resolutions before committing.",
            type: "stderr",
          });
        } else if (gitState.c2ConflictResolved && gitState.c2StagedResolved) {
          const hasSignoff = tokens.includes("-s") || tokens.includes("--signoff");
          if (hasSignoff) {
            ...
            newLines.push(
              {
                text: "[main f482cda] merge branch 'main' of github.com:owner/repo",
                type: "stdout",
              },
              {
                text: " Signed-off-by: Guest Committer <guest-committer@example.com>",
            ...
            newLines.push({
              text: "Error: Merge commits also require DCO Sign-off. Add the '-s' flag.",
              type: "stderr",
            });
          }
        } else {
        ...
        if (!gitState.c2Committed) {
          ...
          newLines.push({
            text: "Error: Merge conflict is active. Pulling was successful, resolve conflicts first.",
            type: "stderr",
          });
        } else if (gitState.c2ConflictResolved && !gitState.c2CommittedMerge) {
          newLines.push({
            text: "Error: Commit the resolved merge before pushing.",
            type: "stderr",
          });
        } else if (gitState.c2ConflictResolved && gitState.c2CommittedMerge) {
          ...
          newLines.push(
            ...
            {
              text: "✓ Remote verification succeeded! Conflict resolved. Challenge 2 Complete.",
              type: "success",
            },
          );
          setTimeout(() => {
        ...
        if (!gitState.c2PushedInitially) {
          ...
          newLines.push({
            text: "Conflict is already active. Resolve it in src/App.tsx.",
            type: "stderr",
          });
        } else {
        ...
        if ((flag === "--ours" || flag === "--theirs") && targetFile === "src/App.tsx") {
          if (!gitState.c2ConflictActive) {
            newLines.push({
              text: "Error: No merge conflict is active for src/App.tsx.",
              type: "stderr",
            });
          } else {
            setGitState((prev) => ({ ...prev, c2ConflictResolved: true }));
            newLines.push({
              text: `Resolved conflict in src/App.tsx using checkout ${flag}`,
              type: "success",
            });
          }
        } else {
          newLines.push({
            text: "Error: Invalid checkout usage. Use 'git checkout --ours src/App.tsx'",
            type: "stderr",
          });
        }
      } else {
        newLines.push({
          text: `git: subcommand '${sub}' is not supported in Challenge 2.`,
          type: "stderr",
        });
      }
    }
      ...
      if (sub === "status") {
        ...
        if (flag === "-i" && (arg === "HEAD~3" || arg === "main" || arg === "origin/main")) {
          if (gitState.c3RebaseCommitted) {
            newLines.push({
              text: "Interactive rebase already complete. Commit history clean.",
              type: "stdout",
            });
          } else {
            ...
            newLines.push(
              {
                text: "------------------ REBASE INTERACTIVE EDITOR ------------------",
                type: "editor",
              },
              ...
              {
                text: "Task: Squash the two WIP commits (2 and 3) into the feature commit (1).",
                type: "stdout",
              },
          ...
          newLines.push({
            text: "Error: Interactive rebase requires '-i HEAD~3' (or equivalent).",
            type: "stderr",
          });
        }
      } else if (sub === "push") {
        ...
        if (gitState.c3ForcePushed) {
          ...
          newLines.push(
            {
              text: "FAIL: Commit history validation rejected by OpenBridge checks!",
              type: "stderr",
            },
            {
              text: "Reason: Found messy work-in-progress (WIP) commits in push history.\nSolution: Run 'git rebase -i HEAD~3' and squash them into 1 clean commit.",
              type: "stderr",
            },
          );
        } else {
          if (hasForce) {
            ...
            newLines.push(
              ...
              {
                text: "✓ Remote verification succeeded! History is clean. Challenge 3 Complete.",
                type: "success",
              },
            );
            setTimeout(() => {
        ...
        newLines.push({
          text: `git: subcommand '${sub}' is not supported in Challenge 3.`,
          type: "stderr",
        });
      }
    }
  ...
  const handleRebaseEditorInput = (input: string) => {
    ...
    const newLines: LogLine[] = [{ text: cleanInput, type: "input" }];
    ...
    if (!gitState.c3RebaseSquashed) {
      if (
        cleanInput === "squash" ||
        cleanInput === "s" ||
        cleanInput.includes("squash") ||
        cleanInput.includes("s ")
      ) {
        ...
        newLines.push({
          text: "Invalid editor action. Type 'squash' (or 's') to combine, or 'abort'.",
          type: "stderr",
        });
      }
    } else {
      ...
      if (input.length < 5) {
        newLines.push({
          text: "Error: Commit message too short. Provide a clear commit description.",
          type: "stderr",
        });
      } else {
        setGitState((prev) => ({
      ...
      <div className="bg-[#0c0d14]/65 border border-zinc-900 rounded-xl p-4 flex flex-col md:flex-row gap-4 items-start select-none">
        ...
        <div className="space-y-2 flex-grow">
          ...
          <div className="space-y-1 mt-1 pl-1">
            <span className="text-[9.5px] uppercase font-bold text-zinc-400 font-mono tracking-wider block">
              Required Steps:
            </span>
            {CHALLENGES[activeChallenge - 1].steps.map((step, idx) => (
              <div
                key={idx}
                className="flex items-start gap-1.5 text-[10.5px] text-zinc-500 font-mono leading-relaxed"
              >
