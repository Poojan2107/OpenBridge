...
const DIFF_CONFIG: Record<string, { label: string; icon: React.ReactNode; color: string; bg: string; border: string }> = {
  beginner:     { label: "Beginner",     icon: <Zap className="w-3 h-3" />,    color: "text-emerald-400", bg: "bg-emerald-950/20", border: "border-emerald-900/40" },
  intermediate: { label: "Intermediate", icon: <Target className="w-3 h-3" />, color: "text-amber-400",   bg: "bg-amber-950/20",   border: "border-amber-900/40" },
  advanced:     { label: "Advanced",     icon: <Trophy className="w-3 h-3" />, color: "text-rose-400",    bg: "bg-rose-950/20",    border: "border-rose-900/40" },
};
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1 leading-relaxed max-w-xl">
            Get 5 realistic interview questions tailored to your stack. Practice before the real thing.
          </p>
        </div>
      ...
      {profile && (
        <div className="flex flex-wrap items-center gap-2">
          <span className="text-[10px] font-mono uppercase tracking-wider text-zinc-600">Tailored for:</span>
          <span className="px-2 py-0.5 rounded text-[10px] font-mono border border-[#30363d] bg-[#0d1117] text-zinc-400">{profile.interest}</span>
          <span className="px-2 py-0.5 rounded text-[10px] font-mono border border-[#30363d] bg-[#0d1117] text-zinc-400">{profile.level}</span>
          {profile.skills.map((s, i) => (
            <span key={i} className="px-2 py-0.5 rounded text-[10px] font-mono border border-[#30363d] bg-[#0d1117] text-zinc-500">{s}</span>
          ))}
        </div>
      )}
      ...
      {loading && (
        <div className="rounded-xl border border-[#30363d] bg-[#0d1117] p-12 flex flex-col items-center justify-center gap-5">
          ...
          <div className="text-center space-y-1">
            ...
            <p className="text-zinc-600 text-xs font-mono">Analyzing {profile?.skills?.join(", ") || "your skills"} for targeted questions</p>
          </div>
        </div>
      )}
        ...
        <div className="rounded-xl border border-dashed border-[#30363d] bg-[#0d1117] p-12 flex flex-col items-center justify-center gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-zinc-600 text-xs mt-1 max-w-sm leading-relaxed">
              Click "Generate Questions" to get 5 personalized technical questions based on your profile. Practice answering them before your real interview.
            </p>
          </div>
          <div className="flex flex-wrap justify-center gap-2 mt-1">
            {["Hints available", "Difficulty levels", "Interviewer insights"].map(t => (
              <span key={t} className="px-2 py-0.5 rounded-full border border-[#30363d] text-[9px] font-mono text-zinc-600">{t}</span>
            ))}
          </div>
        </div>
      )}
          ...
          {questions.map((q, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  {/* Number */}
                  <div className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-mono font-bold shrink-0 border ${
                    isExpanded
                  ...
                  <div className="flex-grow min-w-0 space-y-1.5">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-mono font-bold border ${diff.bg} ${diff.border} ${diff.color}`}>
                    ...
                    <p className={`text-sm leading-snug transition-colors ${isExpanded ? "text-white font-semibold" : "text-zinc-300"}`}>
                      {q.question}
                    </p>
                  </div>
                ...
                {isExpanded && (
                  <div className="px-4 pb-4 space-y-3 border-t border-[#21262d] pt-3 ml-10 animate-fade-in">
                    ...
                    >
                      {hintRevealed ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-[#30363d] hover:border-zinc-700 bg-[#161b22] text-xs font-mono text-zinc-400 hover:text-zinc-200 transition-all cursor-pointer"
                    >
                      {answerRevealed ? <EyeOff className="w-3.5 h-3.5" /> : <HelpCircle className="w-3.5 h-3.5" />}
                    ...
                    {answerRevealed && (
                      <div className="flex items-start gap-2 p-3 rounded-lg border border-violet-900/30 bg-violet-950/10">
                        ...
                        <span className="text-xs text-violet-300/80 leading-relaxed">{q.whatInterviewersLookFor}</span>
                      </div>
                    )}
                  </div>
                )}
