        ...
        <div className="text-center space-y-6 max-w-md">
          ...
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-2 leading-relaxed">
              <span className="text-zinc-300 font-semibold">@{login}</span> hasn't completed the OpenBridge onboarding yet, or the profile doesn't exist.
            </p>
          </div>
          <a
            href="/"
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-10 space-y-8">
          ...
          <div className="rounded-2xl border border-[#30363d] bg-[#161b22] p-6 sm:p-8 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col sm:flex-row sm:items-center gap-6">
              ...
              <div className="flex-grow min-w-0">
                ...
                <p className="text-zinc-400 text-sm leading-relaxed">
                  ...
                  <a href="/" className="text-blue-400 hover:underline font-semibold">OpenBridge</a>
                  . Specializing in <span className="text-zinc-200 font-semibold">{profile.interest}</span> development.
                </p>

                <div className="flex flex-wrap gap-2 mt-4">
                  <span className={`px-2.5 py-1 rounded-md text-xs font-mono font-bold border ${getLevelColor(profile.level)}`}>
                    {profile.level}
                  </span>
                  {profile.skills.map((s, i) => (
                    <span key={i} className="px-2 py-1 rounded-md text-[11px] font-mono bg-[#0d1117] border border-[#30363d] text-zinc-400">
                      {s}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex gap-3 sm:flex-col sm:items-end shrink-0">
                <div className="text-center bg-[#0d1117] border border-[#30363d] rounded-xl px-4 py-3">
                  <span className="block text-2xl font-bold text-white font-mono">{progressPct}<span className="text-base text-zinc-500">%</span></span>
                  <span className="block text-[9px] font-mono uppercase tracking-wider text-zinc-500 mt-0.5">Roadmap Complete</span>
                </div>
                <div className="text-center bg-[#0d1117] border border-[#30363d] rounded-xl px-4 py-3">
                  <span className="block text-2xl font-bold text-white font-mono">{completedCount}<span className="text-sm text-zinc-500">/{totalTasks}</span></span>
                  <span className="block text-[9px] font-mono uppercase tracking-wider text-zinc-500 mt-0.5">Tasks Done</span>
                </div>
              </div>
            </div>
            ...
            <div className="relative z-10 mt-6 pt-5 border-t border-[#30363d]">
              <div className="flex justify-between text-[10px] font-mono text-zinc-500 mb-2">
                ...
                <span className="text-zinc-300 font-bold">{completedCount} / {totalTasks} milestones</span>
              </div>
        ...
        >
          <div className="rounded-2xl border border-[#30363d] bg-[#161b22] p-6 space-y-4">
            <div className="flex items-center justify-between gap-2">
              <div>
                <span className="block text-[10px] font-mono uppercase tracking-wider text-zinc-500 mb-1">OSS Credential</span>
        ...
        >
          ...
          <div className="rounded-2xl border border-[#30363d] bg-[#161b22] p-6 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-mono uppercase tracking-wider text-zinc-500">Progress</span>
            </div>
            ...
            {roadmap ? (
              <div className="space-y-3">
                {weeks.map((weekKey, wi) => {
                  ...
                  const done = tasks.filter((_, ti) => checkedRoadmapTasks[`${weekKey}-${ti}`]).length;
                  ...
                  return (
                    <div key={weekKey} className="space-y-2">
                      <div className="flex items-center justify-between text-xs font-mono">
                        <div className="flex items-center gap-2">
                          <span className={`w-5 h-5 rounded flex items-center justify-center text-[9px] font-bold border ${
                            isComplete
                          ...
                          <div>
                            <span className={`block font-semibold ${isComplete ? "text-zinc-300" : "text-zinc-400"}`}>
                              {WEEK_LABELS[wi]}
                            </span>
                            <span className="block text-[9px] text-zinc-600">{WEEK_SUBTITLES[wi]}</span>
                          </div>
                        </div>
                        <span className={`text-[10px] font-bold ${isComplete ? "text-emerald-400" : "text-zinc-500"}`}>
                          {done}/{tasks.length}
                        </span>
                      </div>
                        ...
                        {tasks.map((task, ti) => {
                          ...
                          return (
                            <div key={ti} className="flex items-start gap-2 text-[11px]">
                              ...
                              <span className={isChecked ? "text-zinc-500 line-through" : "text-zinc-400 leading-snug"}>
                                {task}
                              </span>
                            </div>
                          );
          ...
          <div className="rounded-2xl border border-[#30363d] bg-[#161b22] p-6 space-y-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-mono uppercase tracking-wider text-zinc-500">Matched Repositories</span>
            </div>
            ...
            {repos && repos.length > 0 ? (
              <div className="space-y-3">
                {repos.slice(0, 3).map((repo, ri) => (
                  <div key={ri} className="p-4 rounded-xl bg-[#0d1117] border border-[#30363d] hover:border-zinc-700 transition-all group">
                    <div className="flex items-start justify-between gap-3 mb-2">
                      ...
                      <div className="text-right shrink-0">
                        <span className="block text-sm font-bold font-mono text-emerald-400">{repo.match}</span>
                        <span className="block text-[9px] text-zinc-600 uppercase font-mono">match</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                      <span className={`px-2 py-0.5 text-[9px] font-mono font-bold border rounded ${
                      ...
                      <span className="text-[9px] text-zinc-600 font-mono">{repo.issues?.length || 0} suggested issues</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        >
          <div className="rounded-2xl border border-[#30363d] bg-gradient-to-r from-[#161b22] to-[#0d1117] p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-zinc-500 mt-1">Join <span className="text-zinc-300 font-semibold">@{login}</span> and thousands of contributors on OpenBridge.</p>
            </div>
            <a
              href="/"
        ...
        <div className="text-center pt-4 pb-2">
          <p className="text-[10px] font-mono text-zinc-700">
            Built with <a href="/" className="text-zinc-600 hover:text-zinc-400 transition-colors font-bold">OpenBridge</a> · Open-source contribution mentor
          </p>
        </div>
