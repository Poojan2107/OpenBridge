...
function LevelUpToast({ info, onDone }: { info: LevelInfo; onDone: () => void }) {
  ...
  return (
    ...
    >
      <div
        className={`flex items-center gap-3 px-5 py-3 rounded-2xl border shadow-2xl ${info.bgColor} ${info.borderColor}`}
      ...
      >
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-[10px] font-mono font-bold uppercase tracking-wider px-1.5 py-0.5 rounded ${info.bgColor} ${info.borderColor} border ${info.textColor}`}
            >
              {info.xp} XP
            </span>
          </div>
          ...
          <p className="text-[10.5px] text-zinc-500 mt-0.5 max-w-[220px] leading-snug">
            {info.tagline}
          </p>
        </div>
      </div>
...
export default function LevelBadge({
  ...
  if (variant === "compact") {
    return (
      <>
        ...
        <div
          className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-mono font-bold ${info.bgColor} ${info.borderColor} ${info.textColor} select-none`}
        >
  ...
  if (variant === "profile") {
    return (
      <div
        className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border ${info.bgColor} ${info.borderColor}`}
      >
  ...
  return (
    <>
      ...
      <div
        className={`rounded-xl border p-5 relative overflow-hidden transition-all duration-300 ${info.bgColor} ${info.borderColor}`}
        style={{ boxShadow: `0 0 30px -8px var(--tw-shadow-color)` }}
      >
        {/* Decorative glow blob */}
        <div
          className={`absolute -top-8 -right-8 w-32 h-32 rounded-full blur-[50px] opacity-20 pointer-events-none bg-gradient-to-br ${info.gradientFrom} ${info.gradientTo}`}
        />

        {/* Header row */}
        <div className="relative z-10 flex items-start justify-between gap-4 mb-5">
          <div>
            <span className="block text-[9px] font-mono uppercase tracking-widest text-zinc-500 mb-1">
              Contributor Rank
            </span>
            <div className="flex items-center gap-2">
              <span className="text-3xl">{info.emoji}</span>
              <div>
                <h3 className={`text-lg font-black tracking-tight leading-none ${info.textColor}`}>
                  {info.name}
                </h3>
                <p className="text-[10.5px] text-zinc-500 mt-0.5 leading-snug max-w-[180px]">
                  {info.tagline}
                </p>
              </div>
            </div>
          ...
          {/* XP counter */}
          <div
            className={`text-right shrink-0 p-2.5 rounded-lg border ${info.borderColor} ${info.bgColor}`}
          >
            <span className={`block text-xl font-black font-mono ${info.textColor}`}>{xp}</span>
            <span className="block text-[8px] font-mono uppercase text-zinc-500 tracking-wider">
              / {MAX_XP} XP
            </span>
          </div>
        </div>
        ...
        <div className="relative z-10 mb-5">
          <div className="flex justify-between text-[9px] font-mono text-zinc-600 mb-1.5">
            ...
            {info.nextLevel ? (
              <span>
                {info.nextLevel.name} in{" "}
                <span className={info.textColor + " font-bold"}>{info.xpToNext} XP</span>
              </span>
            ) : (
        ...
        <div className="relative z-10 grid grid-cols-2 gap-2 mb-5">
          <div className={`p-2.5 rounded-lg border ${info.borderColor} bg-black/20`}>
            <span className="block text-[8px] font-mono uppercase text-zinc-600 tracking-wider">
              Task XP
            </span>
        ...
        <div className="relative z-10">
          <span className="block text-[8px] font-mono uppercase tracking-wider text-zinc-600 mb-2">
            All Ranks
          </span>
          <div className="flex items-center gap-1.5 flex-wrap">
            {LEVELS.map((lvl, i) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex items-center gap-1 px-2 py-1 rounded-md border text-[9px] font-mono font-bold transition-all ${
                      isCurrent
                  ...
                  {i < LEVELS.length - 1 && (
                    <span
                      className={`text-[8px] font-mono ${isUnlocked ? "text-zinc-600" : "text-zinc-800"}`}
                    >
                      ›
                    </span>
                  )}
