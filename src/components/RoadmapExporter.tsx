...
async function renderToCanvas(
  ...
  ctx.font = "9px monospace";
  ctx.textAlign = "left";
  ctx.fillText(`Generated by OpenBridge · ${new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}`, 32, fy);
  ctx.textAlign = "right";
...
function drawInitials(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, text: string) {
          ...
          <div className="bg-[#161b22] border border-[#30363d] rounded-xl p-5 space-y-4 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3 relative z-10">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[11px] text-zinc-500 font-mono">@{login} · {profile.interest}</p>
                <div className={`inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-[9px] font-mono font-bold border ${level.bgColor} ${level.borderColor} ${level.textColor}`}>
                  {level.name} · {xp} XP
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {[
                  ...
                  <div key={l} className="text-center bg-[#0d1117] border border-[#21262d] rounded-lg px-3 py-2 min-w-[52px]">
            ...
            <div className="grid grid-cols-2 gap-2 relative z-10">
              {WEEK_META.map(({ key, label, theme, accent }) => {
                ...
                return (
                  <div key={key} className="bg-[#0d1117] border border-[#21262d] rounded-lg overflow-hidden">
                    <div className="h-0.5 w-full" style={{ background: accent + "66" }}>
                      <div className="h-full rounded-full" style={{ width: `${pct}%`, background: accent }} />
                    </div>
                    <div className="p-2.5">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[9px] font-mono font-bold" style={{ color: accent }}>{label.toUpperCase()}</span>
                        <span className="text-[9px] font-mono text-zinc-600">{done}/{tasks.length}</span>
                      </div>
                      ...
                      <div className="space-y-0.5">
                        {tasks.slice(0, 3).map((t, ti) => {
                          ...
                          return (
                            <div key={ti} className="flex items-center gap-1.5">
                              {isDone
                                ? <CheckCircle2 className="w-2.5 h-2.5 shrink-0" style={{ color: accent }} />
                                : <Clock className="w-2.5 h-2.5 text-zinc-700 shrink-0" />
                              }
                              <span className={`text-[9px] font-mono truncate ${isDone ? "text-zinc-600 line-through" : "text-zinc-400"}`}>
                                {t}
                              </span>
                            </div>
                          );
                        ...
                        {tasks.length > 3 && (
                          <span className="text-[8px] font-mono text-zinc-700">+{tasks.length - 3} more</span>
                        )}
                      </div>
                    </div>
                  </div>
