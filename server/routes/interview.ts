...
router.post("/api/interview", async (req, res) => {
  try {
    ...
    if (!ai) {
      ...
      const isBackend = /backend|node|express|python|django|api|sql|database/i.test(
        `${interest} ${skillList}`,
      );
        ...
        ? [
            {
              ...
              whatInterviewersLookFor:
                "Understanding of referential equality, re-render optimization, and when NOT to use them (premature optimization).",
            },
            {
              question:
                "What is the CSS Box Model? How does `box-sizing: border-box` change the default behavior?",
              difficulty: "beginner",
              topic: "CSS Fundamentals",
              hint: "Draw out the layers: content → padding → border → margin.",
              whatInterviewersLookFor:
                "Clear mental model of how width/height are calculated and why border-box is preferred in modern layouts.",
            },
            {
              question:
                "You notice a component re-renders 50 times per second when the user types in a search box. How would you fix it?",
              difficulty: "intermediate",
              topic: "Performance",
              hint: "Consider debouncing, controlled vs uncontrolled inputs, and React.memo.",
              whatInterviewersLookFor:
                "Systematic debugging approach: identify the cause first (state updates), then apply the right tool (debounce, not just memo everything).",
            },
            {
              question:
                "What is a closure in JavaScript? Give a practical example where closures are useful.",
              difficulty: "beginner",
              ...
              whatInterviewersLookFor:
                "Real understanding, not textbook definition. Bonus points for mentioning event handlers, data privacy, or factory functions.",
            },
            {
              question:
                "Walk me through what happens from the moment a user types a URL in the browser to seeing the rendered page.",
              difficulty: "advanced",
              topic: "Web Fundamentals",
              hint: "DNS → TCP → TLS → HTTP request → server response → HTML parsing → CSSOM → render tree → paint.",
              whatInterviewersLookFor:
                "Breadth of knowledge across networking, browser internals, and rendering pipeline. Depth at any single step is a bonus.",
            },
          ]
        : isBackend
          ? [
              {
                ...
                whatInterviewersLookFor:
                  "Nuanced answer — not 'NoSQL is faster'. They want to hear about trade-offs: ACID vs eventual consistency, schema flexibility vs query power.",
              },
              {
                question:
                  "Explain what middleware is in Express.js. Write a simple logging middleware.",
                difficulty: "beginner",
                topic: "Node.js / Express",
                hint: "Middleware is a function with (req, res, next) that sits between the request and your route handler.",
                whatInterviewersLookFor:
                  "Understanding of the request pipeline, the importance of calling next(), and practical use cases (auth, logging, CORS).",
              },
              {
                ...
                whatInterviewersLookFor:
                  "Awareness of concurrency issues even in single-threaded Node.js (async operations). Mention of solutions: locks, optimistic concurrency, transactions.",
              },
              {
                question:
                  "How would you design a rate limiter for an API? What data structures would you use?",
                difficulty: "intermediate",
                topic: "System Design",
                hint: "Consider sliding window, token bucket, or fixed window counters. Think about storage (in-memory vs Redis).",
                whatInterviewersLookFor:
                  "Systematic thinking: define the constraint, choose algorithm, discuss trade-offs, mention distributed concerns.",
              },
              {
                ...
                whatInterviewersLookFor:
                  "Practical experience with ORMs, understanding of query optimization, and ability to spot performance anti-patterns.",
              },
            ]
          : [
              {
                question:
                  "What is the difference between a stack and a queue? Give a real-world analogy for each.",
                difficulty: "beginner",
                topic: "Data Structures",
                hint: "Stack = plate stack (LIFO). Queue = checkout line (FIFO).",
                whatInterviewersLookFor:
                  "Clear analogies and understanding of when to use each in code (undo systems, BFS vs DFS, etc.).",
              },
              {
                ...
                whatInterviewersLookFor:
                  "Intuitive understanding, not just memorized definitions. Can they look at code and estimate its complexity?",
              },
              {
                question:
                  "What is version control? Why do open-source projects require pull requests instead of direct commits?",
                difficulty: "beginner",
                topic: "Git & Collaboration",
                hint: "Think about code review, history tracking, and collaboration safety.",
                whatInterviewersLookFor:
                  "Understanding of collaborative development workflows, not just git commands.",
              },
              {
                question:
                  "Describe the SOLID principles. Pick one and explain how violating it causes problems.",
                difficulty: "intermediate",
                topic: "Software Design",
                hint: "S = Single Responsibility, O = Open/Closed, L = Liskov Substitution, I = Interface Segregation, D = Dependency Inversion.",
                whatInterviewersLookFor:
                  "Practical understanding with a real example, not just reciting the acronym.",
              },
              {
                question:
                  "How would you debug a function that returns the wrong output but doesn't throw an error?",
                difficulty: "intermediate",
                topic: "Debugging",
                hint: "Systematic approach: check inputs, add breakpoints/logs at key steps, isolate the failing logic, write a test case.",
                whatInterviewersLookFor:
                  "Methodical debugging process rather than random guessing. Mention of tools (debugger, logs, tests).",
              },
            ];
