...
const CodeReviewSchema = z.object({
  code: z
    .string()
    .min(10, "Please provide at least 10 characters of code.")
    .max(8000, "Code too long. Please paste under 8000 characters."),
  ...
  try {
    ...
    if (!ai) {
      ...
      return res.json({
        ...
        praise: [
          hasComments
            ? "Code includes comments, which helps reviewers understand intent."
            : "Code is concise and readable.",
          hasTypeAnnotations
            ? "Type annotations present — great for maintainability."
            ...
            ? [
                {
                  severity: "warning",
                  message:
                    "Remove debug console.log() statements before merging — maintainers will flag these.",
                  suggestion: "Use a proper logger or remove entirely before opening the PR.",
                },
              ]
            : []),
          ...(hasTodos
            ? [
                {
                  severity: "info",
                  message: "TODO/FIXME comments found in the diff.",
                  suggestion:
                    "Either resolve them now or open a follow-up issue referencing the TODO.",
                },
              ]
            : []),
          {
            severity: "info",
            message: "No automated tests included with this change.",
            suggestion:
              "Add a unit test covering the happy path. Even one test dramatically increases merge likelihood.",
          },
        ],
        suggestions: [
          ...
          hasTypeAnnotations
            ? "Consider adding JSDoc comments to public functions for documentation generators."
            : "Add type annotations to function parameters to help TypeScript catch bugs early.",
        ],
      });
    }
        ...
        responseSchema: {
          ...
          properties: {
            ...
            issues: {
              ...
              items: {
                type: Type.OBJECT,
                required: ["override", "severity", "message", "suggestion"].filter(
                  (k) => k !== "override",
