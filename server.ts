...
// POST /api/codereview — AI-powered beginner-friendly code review
app.post("/api/codereview", rateLimiter, async (req, res) => {
  try {
    const { code = "", language = "auto", context = "" } = req.body;

    if (!code.trim() || code.trim().length < 10) {
      return res.status(400).json({ error: "Please provide at least 10 characters of code." });
    }

    if (code.length > 8000) {
      return res.status(400).json({ error: "Code too long. Please paste under 8000 characters." });
    }
...
// POST /api/interview — Generate tailored mock interview questions
app.post("/api/interview", rateLimiter, async (req, res) => {
  try {
    const { skills = [], level = "Beginner", interest = "Frontend" } = req.body;
...
app.get("/api/badge/:login.svg", async (req, res) => {
  try {
    const { login } = req.params;
    ...
    if (user?.profile?.roadmap?.weeks) {
      ...
      // Mock progress fallback for preview if database has no tasks yet
      completedCount = 3;
    }
